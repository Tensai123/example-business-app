...
dependency "security" {
  ...
  mock_outputs = {
    ...
    sql_admin_password_value          = "mock-password-value"
  }
  mock_outputs_allowed_terraform_commands = ["plan", "validate", "destroy"]
}
...
inputs = {
  ...
  # Dependencies from security (Key Vault password)
  sql_admin_password                = dependency.security.outputs.sql_admin_password_value
  key_vault_password_secret_id      = dependency.security.outputs.sql_admin_password_secret_id
  
  # Dependencies from networking (for VNet rules)
  allowed_subnet_ids = [
